...
class UserRegistrationForm(UserCreationForm):
    ...
    address = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'form-control py-4', 'placeholder': 'Введите адрес'}))
    birthday = forms.DateField(
        widget=forms.DateInput(
            attrs={'class': 'form-control py-4', 'placeholder': 'Введите дату рождения (ГГГГ-ММ-ДД)'}))
    ...
    class Meta:
        model = Users
        fields = (
            'first_name', 'last_name', 'username', 'email', 'phone', 'address', 'birthday', 'password1', 'password2')
